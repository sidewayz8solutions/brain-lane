...
export default function DiffViewer({ diff }) {
    ...
    const handleCommitToGitHub = async () => {
        ...
        const repo = window.prompt('Enter repository (owner/repo), e.g. sidewayz8solutions/brain-lane');
        if (!repo) return;

        const baseBranch = window.prompt('Base branch to create PR from (default: main)', 'main') || 'main';
        const prTitle = window.prompt('PR title', diff.explanation ? `Auto-fix: ${diff.explanation.slice(0, 80)}` : 'Auto-fix by Brain Lane') || 'Auto-fix by Brain Lane';

        const newBranch = `brain-lane/auto-fix-${Date.now()}`;

        setIsProcessing(true);
        setPrUrl(null);

        try {
            await githubService.createBranch(repo, baseBranch, newBranch);

            for (const file of diff.files) {
                const path = file.path;
                const content = file.modified ?? file.newContent ?? '';
                await githubService.createOrUpdateFile(repo, newBranch, path, content, `${prTitle} - ${path}`);
            }

            const pr = await githubService.createPullRequest(repo, baseBranch, newBranch, prTitle, diff.explanation || 'Automated fix generated by Brain Lane');

            const url = pr.html_url || pr.url;
            setPrUrl(url);
            alert('Pull request created: ' + url);
        } catch (err) {
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                    <button
                        className="px-3 py-1 rounded bg-cyan-600 text-xs text-white hover:bg-cyan-500 disabled:opacity-50"
                        onClick={handleCommitToGitHub}
                        disabled={isProcessing}
                    >
                        {isProcessing ? 'Creating PR...' : 'Commit to GitHub'}
                    </button>
                </div>
            </div>
